...
En estos razonamientos se puede cambiar un poco cómo se contabiliza la cantidad de instrucciones sin que el orden varíe. Por ejemplo, en el último algoritmo podemos contabilizar la evaluación de las condiciones de los {\tt if} como instrucciones, llegando a la recurrencia distinta $a_0=2$ y $a_n = 4 + a_{n-1}$. Sin embargo, esto solamente cambia constantes de la fórmula final que no afectan el orden, pues queda $a_n = 2 + 4n$ que también es $O(n)$. En estos casos considerar los {\tt if} o no es una decisión arbitraria.
\end{document}
